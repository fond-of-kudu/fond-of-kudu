...
class AvailableSortMapExpanderPluginTest extends Unit
{
    ...
    /**
     * @var \FondOfKudu\Zed\CatalogSearchConnector\Communication\Plugin\ProductAbstractMapExpander\AvailableSortMapExpanderPlugin
     */
    ...
    {
        $productData = [
            CatalogSearchConnectorConstants::ATTR_AVAILABLE => 1,
        ];

        $this->pageMapBuilderMock->expects(static::once())
            ->method('addIntegerSort')
            ->with($this->pageMapTransferMock, CatalogSearchConnectorConstants::ATTR_AVAILABLE, 1)
            ->willReturnSelf();
