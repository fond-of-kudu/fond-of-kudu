...
class IsSoldOutMapExpanderPluginTest extends Unit
{
    ...
    /**
     * @var \FondOfKudu\Zed\CatalogSearchConnector\Communication\Plugin\ProductAbstractMapExpander\IsSoldOutMapExpanderPlugin
     */
    ...
    {
        $productData = [
            CatalogSearchConnectorConstants::ATTR_IS_SOLD_OUT => 'yes',
        ];

        $this->pageMapTransferMock->expects(static::once())
            ->method('setIsSoldOut')
            ->with($productData[CatalogSearchConnectorConstants::ATTR_IS_SOLD_OUT])
            ->willReturnSelf();

        $this->pageMapBuilderMock->expects(static::once())
            ->method('addStringSort')
            ->with($this->pageMapTransferMock, CatalogSearchConnectorConstants::ATTR_IS_SOLD_OUT, 'yes')
            ->willReturnSelf();
